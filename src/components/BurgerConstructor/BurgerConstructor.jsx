...
function BurgerConstructor() {
  ...
  const isOrderModalOpen = useSelector(
    (state) => state.orderData.isOrderDetailsModalOpen
  );
  ...
  const handleDrop = (item) => {
    const selectedIngredient = burgerData.find(
  ...
  const moveIngredients = useCallback(
    (dragIndex, hoverIndex, selectedIngredients) => {
  ...
  return (
    <section
      className={`${styles.section} ${isHover && styles.dropping}`}
      ref={dropRef}
    >
      ...
      {selectedIngredients.length > 0 ? (
        <div className={"mr-4 " + styles.total}>
          <span className={"text text_type_digits-medium mr-10 " + styles.sum}>
            {sum}
            {<CurrencyIcon />}
          </span>
          <Button
