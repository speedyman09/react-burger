...
function BurgerIngredients() {
  const isIngredientModalOpen = useSelector(
    (state) => state.ingredients.isIngredientModalOpen
  );
  ...
  const handleScroll = () => {
    ...
    const bunsOffset = Math.abs(
      bunRef.current.getBoundingClientRect().y - lineY
    );
    const sauceOffset = Math.abs(
      sauceRef.current.getBoundingClientRect().y - lineY
    );
    const mainsOffset = Math.abs(
      mainRef.current.getBoundingClientRect().y - lineY
    );

    if (bunsOffset < sauceOffset && bunsOffset < mainsOffset)
      setCurrent("buns");
    if (sauceOffset < bunsOffset && sauceOffset < mainsOffset)
      setCurrent("sauces");
    if (mainsOffset < bunsOffset && mainsOffset < sauceOffset)
      setCurrent("mains");
  };
  ...
  return (
    <section className={styles.ingredients}>
      <h2
        className={
          "text text_type_main-large text text_color_primary mt-10 mb-5"
        }
      >
        Соберите бургер
      </h2>
      ...
      <div className={styles.wrapper}>
        <div
          className={styles.scroll + " custom-scroll pt-10"}
          ref={scrollRef}
        ...
        >
          ...
          <IngredientCategory
            ...
            category={mains}
            heading={"Начинки"}
            listStyle={`pl-4 pr-4 pb-8 ${styles.list} ${
              selectedIngredients.length === 0 && styles.disabled
            }`}
            textStyle={textStyle + " pt-10"}
          />
